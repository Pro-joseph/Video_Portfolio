<header class="fixed top-0 left-0 right-0 z-50" style="background: var(--header-bg); backdrop-filter: blur(12px); border-bottom: 1px solid var(--border);">
    <div class="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
        ...
        <nav id="mobileMenu" style="display: none; position: fixed; top: 65px; left: 0; right: 0; background: var(--header-bg); backdrop-filter: blur(12px); border-bottom: 1px solid var(--border); padding: 1.5rem 2rem; flex-direction: column; gap: 1.25rem; z-index: 40;">
            <a href="{{ route('home') }}" class="nav-link" style="{{ request()->is('/') ? 'color: var(--orange);' : '' }} font-size: 1.1rem;">Home</a>
            <a href="{{ route('portfolio') }}" class="nav-link" style="{{ request()->is('portfolio*') ? 'color: var(--orange);' : '' }} font-size: 1.1rem;">Work</a>
            <a href="{{ route('reels') }}" class="nav-link" style="{{ request()->is('reels') ? 'color: var(--orange);' : '' }} font-size: 1.1rem;">Reels</a>
            <a href="{{ route('gallery') }}" class="nav-link" style="{{ request()->is('gallery*') ? 'color: var(--orange);' : '' }} font-size: 1.1rem;">Gallery</a>
            <a href="{{ route('about') }}" class="nav-link" style="{{ request()->is('about') ? 'color: var(--orange);' : '' }} font-size: 1.1rem;">About</a>
            <a href="{{ route('contact') }}" class="btn-primary text-sm" style="padding: 0.65rem 1.5rem; font-size: 0.85rem; text-align: center;">Let's Talk</a>
        </nav>
    </div>
</header>
