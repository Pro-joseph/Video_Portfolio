    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($reels as $reel)
                <a href="{{ route('portfolio.show', $reel) }}" class="reel-card card-fade" style="transition-delay: {{ $loop->index * 100 }}ms;">
                    <div style="aspect-ratio: 9/16; position: relative; overflow: hidden;">
                        ...
                        @endphp
                        <img src="{{ $thumb }}" alt="{{ $reel->title }}" class="w-full h-full object-cover" style="transition: transform 0.6s ease;" onerror="this.src='https://images.unsplash.com/photo-1574717024653-61fd2cf4d44d?w=400&q=80'">
                        <div style="position: absolute; inset: 0; background: linear-gradient(to top, rgba(10,10,10,0.8) 0%, transparent 50%);"></div>
                        <div style="position: absolute; bottom: 0; left: 0; right: 0; padding: 1rem;">
                            <h3 style="font-family: 'Bebas Neue', sans-serif; font-size: 1.4rem; color: white; letter-spacing: 0.03em;">{{ $reel->title }}</h3>
                        </div>
